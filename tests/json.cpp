...
template<typename T>
void CHECK_OK(const std::expected<T, auxiliary::JsonErrorCode>& r) {
	using enum auxiliary::JsonErrorCode;
	CHECK(r.has_value());
}
...
template<typename T>
void CHECK_READ_ERROR(auxiliary::JsonReader& r, auxiliary::u8string_view key, auxiliary::JsonErrorCode code) {
	T value;
